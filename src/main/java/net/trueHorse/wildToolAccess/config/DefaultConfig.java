...
public class DefaultConfig {
    ...
    private static Properties createConfigComments(){
        ...
        comments.setProperty("mouseSelect", "#This mods GitHib page: https://github.com/TrueHorse/Wild-Tool-Access \n"+
                                            "#if you want to be able to select an item in the access bar by left clicking");
        ...
        comments.setProperty("labels", "#what information should be shown about the items  all->all; enchantments-> enchantments/potion effect and name;\n"+
                                        "#name->name; non->non");
        comments.setProperty("lastSwappedOutFirst", "#if the tool swapped out should be shown in the first access bar slot next time");
        comments.setProperty("moveIfNextEmpty", "#if the item that would be swapped out of your hotbar should move to the right instead, if that slot is empty");
